	...
	if platform.system() == 'Linux':
		...
		if inp == '0' or inp == '2':
			...
			if os.path.exists(PATH+'/DeployV2/Swarm/credentials-docker.txt') and os.path.exists(PATH+'/DeployV2/Swarm/key-docker.key'):
				...
				if os.path.exists(PATH+'/fogB1/target/docker/Dockerfile') and os.path.exists(PATH+'/fogB1/target/docker/fogB1-1.0-SNAPSHOT-thorntail.jar'):
					if local_registry == True:
						os.system('cd "'+PATH+'/fogB1/target/docker"; docker buildx build --builder=builder --platform linux/amd64,linux/arm64 -t '+ip+':5000/hil:fogb1 . --push')
					else:
						os.system('cd "'+PATH+'/fogB1/target/docker"; docker buildx build --builder=builder --platform linux/amd64,linux/arm64 -t '+usr+'/hil:fogb1 . --push')
				else:
				...
				if os.path.exists(PATH+'/SensB1/target/docker/Dockerfile') and os.path.exists(PATH+'/SensB1/target/docker/SensB1-1.0-SNAPSHOT-thorntail.jar'):
					if local_registry == True:
						os.system('cd "'+PATH+'/SensB1/target/docker"; docker buildx build --builder=builder --platform linux/amd64,linux/arm64 -t '+ip+':5000/hil:sensor-sensb1 . --push')
					else:
						os.system('cd "'+PATH+'/SensB1/target/docker"; docker buildx build --builder=builder --platform linux/amd64,linux/arm64 -t '+usr+'/hil:sensor-sensb1 . --push')
				else:
				...
				if os.path.exists(PATH+'/ActB1/target/docker/Dockerfile') and os.path.exists(PATH+'/ActB1/target/docker/ActB1-1.0-SNAPSHOT-thorntail.jar'):
					if local_registry == True:
						os.system('cd "'+PATH+'/ActB1/target/docker"; docker buildx build --builder=builder --platform linux/amd64,linux/arm64 -t '+ip+':5000/hil:actuator-actb1 . --push')
					else:
						os.system('cd "'+PATH+'/ActB1/target/docker"; docker buildx build --builder=builder --platform linux/amd64,linux/arm64 -t '+usr+'/hil:actuator-actb1 . --push')
				else:
