...
if paramiko_found and scp_found and cryptography_found:
	...
	print ('\n----------------------------------------------------------------------------------------------------\n')
	print ("Starting to build a Docker Swarm deployment ('load-swarm-worker-fogA0.py')")
	print ('\n----------------------------------------------------------------------------------------------------\n')
	if platform.system() == 'Linux':
		...
		if os.path.exists(os.path.dirname(PATH)+'/DeployV2/Swarm/join-manager-cmd.txt'):
			...
			with open(PATH+'/credentials-fogA0.txt', 'w') as file:
				if inp == 'yes':
					file.write(inp+'\n'+username+'\n'+password)
				else:
					file.write(inp+'\n'+username)
			key = Fernet.generate_key()
			with open(PATH+'/key-fogA0.key', 'wb') as file:
				file.write(key)
			with open(PATH+'/credentials-fogA0.txt', 'rb') as file:
				decrypted_credentials = file.read()
			encrypted_credentials = Fernet(key).encrypt(decrypted_credentials)
			with open(PATH+'/credentials-fogA0.txt', 'wb') as encrypted_file:
			...
			if inp == 'yes':
				...
				scp = subprocess.Popen(f"scp '"+os.path.dirname(PATH)+"/DeployV2/Swarm/daemon.json' "+username+"@158.49.245.143:/home/pi", shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
				scp.wait()
				scp = subprocess.Popen(f"scp '"+os.path.dirname(PATH)+"/DeployV2/docker-run.yaml' "+username+"@158.49.245.143:/home/pi", shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
				scp.wait()
	...
	print ('\n----------------------------------------------------------------------------------------------------\n')
	print ("End of Docker Swarm deployment ('load-swarm-worker-fogA0.py')")
else:
	print ("\nThe required libraries are not installed. Please, rerun the script again ('load-swarm-worker-fogA0.py')")
print ('\n----------------------------------------------------------------------------------------------------\n')
